...
```
###### while 반복문을 시작하면 fill 함수가 나오는데 이때 위에서 선언해준 int형 배열 d를 -1로 초기화 해준다 여기서 d 배열을 선언해준 이유가 나오는데 d 배열은 간선으로 유량이 이동하지 않았을때 -1 인 상태를 유지해주기 위함이다. true와 false로 구성된 bool 함수라 생각하면 이해하기가 편하다.

###### bool 함수와 다른점은 d 배열에 연결된 간선 정보를 저장해주는점이 다르다. 이때 연결된 간선 정보를 이용해 
```
int flow = INF;
for(int i = end; i != start; i = d[i]) {
	flow = min(flow, c[d[i]][i] - f[d[i]][i]);
